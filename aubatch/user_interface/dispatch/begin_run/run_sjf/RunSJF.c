...
int run_sjf(int nargs, char **args, int *exit_cmd,
    command_data_struct *command_data)
{
  if (*(command_data->policy) == 1) {
    return 0;
  }
  *(command_data->policy) = 1;
  ...
  for (int i=0; i < *(command_data->count); i++)
  {
    ...
    long int min_burst = command_data->jobBuffer[tmp_buf_tail].burst;
    for (int j = i; j < limit; j++)
    {
      long int tmp = command_data->jobBuffer[tmp_buf_tail].burst;
      if (tmp < min_burst)
      {
        min_burst = tmp;
